...
| タブ | 地図に出るもの |
| --- | --- |
| 周辺施設 | ON にしたカテゴリの施設ピン |
| 土地・災害 | 避難場所ピン。洪水・内水・高潮・津波・土砂・犯罪発生チップを ON にするとレイヤーと**凡例**（地図左上とチップ下）を表示 |
| 交通・学区・人口 | 最寄り駅2つ（半径外でも）・最寄りバス停2つ・学区の小中学校・役所・図書館 |
| 価格 | 地価公示・地価調査の地点（価格バッジ付き） |
| 嫌悪施設 | タブを開くと自動で半径300mを探索し、種別ごとのピンを表示 |
...
| 嫌悪施設（パチンコ・工場・ガソリンスタンド・風俗・ごみ処理・下水処理・火葬場・墓地/霊園・神社/寺・葬儀場・物流・変電所・ガスタンク・畜産）| Google Places Nearby（タイプ）＋ Text Search（13語）＋ **OpenStreetMap Overpass API**（寺社・墓地・変電所などタグで網羅）→ 名称・業種・タグで分類し重複除去。課金単価が高いため嫌悪施設タブを開いたときだけ取得（半径300m、再探索はボタン） | Google（OSM はキー不要） |
...
│   ├── MapLegend.tsx         # 表示中レイヤーの凡例（地図左上のコンパクト版とチップ下の説明版）
